---
name: review-code
description: Code review using 4 calibrated perspectives — no plan context
argument-hint: "[all | file-or-diff-scope]"
allowed-tools:
  - Read
  - Glob
  - Grep
...
- If default scope has no unstaged changes but staged changes exist, tell the user: "No unstaged changes found. Run `/review-code all` to include staged changes." and stop.
...
Check conversation context for previous `## Code Review Summary - Pass` outputs:
- None found → **Pass 1**
